...
const soporteBtn = document.querySelectorAll(".soporte-btn");
...
const generateBotResponse = async (incomingMessageDiv) => {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:3000/chatbot`, requestOptions);
        const data = await response.json();
        if (!response.ok) throw new Error(data.error.message);

        //Extract and display bot's response text
        //Extraer y mostrar el texto de respuesta del bot
        const apiResponseType = data.response.tipo
        const apiResponseText = data.response.data;

        if (apiResponseType === "html") {
            messageElement.innerHTML = apiResponseText;
        }

        if (apiResponseType === "text") {
            messageElement.innerText = apiResponseText;
        }

        if (apiResponseType === "db") {
            messageElement.innerText = apiResponseText;
        }
...
const handleOutgoingMessage = (e, msj) => {
    e.preventDefault();
    userData.message = msj || messageInput.value.trim();
    messageInput.value = "";
...
chatBody.addEventListener("click", (e) => {
    if (e.target.classList.contains("soporte-btn")) {
        handleOutgoingMessage(e, "soporte");
    }
});